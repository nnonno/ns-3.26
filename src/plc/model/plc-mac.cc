...
PLC_Mac::CcaConfirm(PLC_PhyCcaResult status)
{
	...
	if (m_csmaca_active)
	{
		...
		{
			Simulator::ScheduleNow(&PLC_Mac::CsmaCaConfirm, this, PLCCHANNEL_IDLE);
		}
		else
		{
			...
			if (m_NB > m_macMaxCSMABackoffs)
			{
				...
				Simulator::ScheduleNow(&PLC_Mac::CsmaCaConfirm, this, PLCCHANNEL_ACCESS_FAILURE);
			}
			else
			{
...
PLC_Mac::SetAddress (Mac48Address addr)
{
	...
	while  (mac_list.find (addr) != mac_list.end ()) // Changed if to while and added functionailty to increment duplicated MAC instead of abort
	{
		//NS_ABORT_MSG ("PLC_Mac::SetAddress: address " << addr << " has already been asigned to another device!");
                NS_LOG_INFO ("PLC_Mac::SetAddress: address " << addr << " has already been asigned to another device! Incrementing address");
                uint8_t buf[6];
	        (&addr)->CopyTo(buf);
