...
#define CCA_THRESHOLD_POWER 1e-9 // mW
...
class ModulationAndCodingScheme
{
public:
	ModulationAndCodingScheme (void) { mct = BPSK_1_2; gap2Capacity_dB = 0; }
	ModulationAndCodingScheme (ModulationAndCodingType t, double g) { mct = t; gap2Capacity_dB = g; }

	ModulationAndCodingType mct;
	double gap2Capacity_dB;
};

std::ostream& operator<<(std::ostream& os, ModulationAndCodingType mct);
std::ostream& operator<<(std::ostream& os, ModulationAndCodingScheme mcs);

double GetCodeRate (ModulationAndCodingType mct);
size_t GetBitsPerSymbol (ModulationAndCodingType mct);
...
{
	PLC_Mutex() {
//		std::cout << "Creating PLC_Mutex " << &lock << std::endl;
		omp_init_lock(&lock);
	}
	...
	void Lock() const
	{
//		std::cout << "Thread " << omp_get_thread_num() << ": locking " << &lock << std::endl;
		omp_set_lock(&lock);
//		std::cout << "Thread " << omp_get_thread_num() << ": locked " << &lock << std::endl;
	}

	void Unlock() const
	{
//		std::cout << "Thread " << omp_get_thread_num() << ": unlocking " << &lock << std::endl;
		omp_unset_lock(&lock);
//		std::cout << "Thread " << omp_get_thread_num() << ": unlocked " << &lock << std::endl;
	}
...
struct PLC_Mutex
{
	...
	typedef std::map<Timeslot, std::pair<Timeslot, Ptr<const SpectrumValue> > >	PLC_RxPsdMap;
