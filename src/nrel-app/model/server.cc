...
double Server::GetThroughput () const
{
  NS_LOG_FUNCTION (this);
  return m_throughput;
}

double Server::GetLatency () const
{
  NS_LOG_FUNCTION (this);
  return m_delay;
}
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("At time " << Simulator::Now ().GetSeconds ()<< "s Server received "<<  m_totalRx << " bytes from Client "<<m_id);
  ...
  while ((packet = socket->RecvFrom (from)))
    {
      ...
      m_totalRx += (packet->GetSize ()+12);
      m_throughput = m_totalRx*8000/(Simulator::Now().GetMilliSeconds() - m_initTx)/1000;
      m_delay=Simulator::Now().GetMilliSeconds() - QosTx.GetTs().GetMilliSeconds();
      if(m_delay>0){
        if(m_Maxlatency<=m_delay)
          m_Maxlatency = m_delay;
        
        m_latency = (m_latency*m_received + m_delay)/(m_received+1);
        m_received++;       
      }
